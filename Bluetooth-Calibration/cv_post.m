function cv_direction_vectors = cv_post(bag)
    ...
    for i = 1:length(camera_msgs)
        ...
        [id,~,pose] = readAprilTag(camera_matrix, "tag36h11",intrinsics,marker_size);
        for idx = 1:length(id)
            if (id ~= 5)
                disp("Got weird april id? : " + id)
            end
        ...
        [id,~,pose] = readArucoMarker(camera_matrix, "DICT_6X6_50",intrinsics,marker_size);
        for idx = 1:length(id)
            if (id ~= 5)
